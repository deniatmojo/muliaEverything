...
const ROUTES = {
  ...
  // === Modul SO: master, versi BOQ, approval ===
  SO_LIST:              { method: 'GET',  path: () => '/so' },
  SO_GET_KURS:          { method: 'GET',  path: () => '/so/kurs' },
  SO_CREATE:            { method: 'POST', path: () => '/so/create' },
  SO_DETAIL:            { method: 'GET',  path: (d) => `/so/${encodeURIComponent(d.soId)}/detail` },
  SO_DELETE:            { method: 'DELETE', path: (d) => `/so/${encodeURIComponent(d.soId)}` },
  SO_UPLOAD_VERSION:    { method: 'POST', path: (d) => `/so/${encodeURIComponent(d.soId)}/upload` },
  SO_MANUAL_SUBMIT:     { method: 'POST', path: (d) => `/so/${encodeURIComponent(d.soId)}/manual-submit` },
  SO_PENDING_CHANGES:   { method: 'GET',  path: () => '/so/changes/pending' },
  SO_CHANGES_HISTORY:   { method: 'GET',  path: () => '/so/changes/history' },
  SO_CHANGE_DETAIL:     { method: 'GET',  path: (d) => `/so/changes/${encodeURIComponent(d.id)}` },
  SO_CHANGE_SUBMIT:     { method: 'POST', path: (d) => `/so/changes/${encodeURIComponent(d.id)}/submit` },
  SO_CHANGE_APPROVE:    { method: 'POST', path: (d) => `/so/changes/${encodeURIComponent(d.id)}/approve` },
  SO_CHANGE_REJECT:     { method: 'POST', path: (d) => `/so/changes/${encodeURIComponent(d.id)}/reject` },
  SO_CHANGE_DISCARD:    { method: 'POST', path: (d) => `/so/changes/${encodeURIComponent(d.id)}/discard` },
